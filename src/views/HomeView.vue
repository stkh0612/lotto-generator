  ...
  <v-container class="home-view py-6" fluid>
    ...
    <v-card class="glass-card pa-6 mx-auto mt-10 text-left" max-width="600" style="background: rgba(124, 77, 255, 0.03) !important;">
      <h2 class="text-subtitle-1 font-weight-bold text-primary mb-3">
        LottoMate 로또 번호 생성기 및 필터 알고리즘 안내
      </h2>
      <div class="text-caption text-grey-darken-2" style="line-height: 1.6;">
        <p class="mb-3">
          로또메이트(LottoMate)는 과학적인 난수 발생 알고리즘을 활용한 <strong>로또 번호 생성기</strong>입니다. 단순한 무작위 추출에 머무르지 않고, 역대 로또 당첨 데이터 전체와 대조하여 이미 과거에 1등으로 출현했던 번호 조합은 자동으로 거르고 당첨 확률을 높일 수 있는 필터링 기능이 기본적으로 작동합니다.
        </p>
        <p class="mb-3">
          본 로또 추천 조합기 서비스는 45개의 숫자 분포를 고르게 섞는 물리 시뮬레이션 추첨기 엔진을 시각적으로 구현하여, 실제 추첨 방송의 긴장감을 웹 브라우저에서 생생하게 즐길 수 있습니다. 번호를 추출할 때마다 과거 당첨 번호 통계와 비교하여 더욱 신선하고 가능성 높은 행운의 조합을 찾아냅니다.
        </p>
        <p>
          수학적으로 매 회차의 로또 확률은 독립 시행이지만, 균형 잡힌 번호 조합(홀짝 비율 3:3 또는 2:4, 번호 합계가 100~170 사이인 조합 등)을 생성하는 것이 심리적 만족도를 극대화합니다. 로또메이트 무료 생성기로 오늘의 대박 행운을 점쳐보고 저장된 번호를 PNG 이미지로 소장하여 로또 명당에 방문해 보세요!
        </p>
      </div>
    </v-card>
